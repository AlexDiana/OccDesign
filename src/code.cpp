...
double loglik_cpp(arma::vec pars,
  ...
  arma::vec beta_psi = pars.elem(cov_psi);
  arma::vec beta_p = pars.elem(cov_p);
  
  arma::vec Xpsibetapsi = X_psi * beta_psi;
  arma::vec Xpbetap = X_p * beta_p;
  ...
  double log_grad = - (1.0 / (2 * 4)) * (sum(beta_psi % beta_psi) + 
                       sum(beta_p % beta_p));
